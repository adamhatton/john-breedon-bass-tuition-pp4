...
class AccountPage(View):
    ...
    def get(self, request):
        ...
        '''
        bookings = Booking.objects.filter(user=request.user).filter(
            date__gt=datetime.date.today()
        )
        user_form = UserForm(instance=request.user)
        learner_profile_form = LearnerProfileForm(
            instance=request.user.learnerprofile
        )

        if Testimonial.objects.filter(user=request.user).exists():
            testimonial_form = TestimonialForm(
                instance=request.user.testimonial
            )
            user_has_testimonial = True
            testimonial_approved = (
                Testimonial.objects.get(user=request.user).approved
            )
        else:
            testimonial_form = TestimonialForm()
    ...
    def post(self, request):
        ...
        learner_profile_form = LearnerProfileForm(
            request.POST, instance=request.user.learnerprofile
        )
        ...
        # If form isn't valid then return form with enabled fields
            ...
            learner_profile_form.helper.layout = (
                learner_profile_form.enabled_layout
            )
